...
SELECT SYSTIMESTAMP FROM DUAL; -- 2025-03-10 12:12:38.733 +0900
...
TO_CHAR(SYSDATE, '(DY)') AS 오늘날짜 FROM DUAL;
*/
...
-- R패턴 : 1세기 기준으로 절반(50년) 이상인 경우 이전세기(1900년대)
...
SELECT EMP_NAME, SALARY, NVL(BONUS, 0), SALARY * NVL(EMPLOYEE.BONUS , 0)
FROM EMPLOYEE;
...
-- 그룹 함수
-- 하나 이상의 행을 그룹으로 묶어 연산하여 총합, 평균 드으이
-- 하나의 결과 행으로 반환하는 함수
...
-- 급여 최소값, 가장빠른 입사일, 알파벳 순서가 가장 빠른 이메일 조회
