...
-- 여러개의 결과값을 산출하기 위해 그룹함수가 적용된 기준 ORDER BY 절에 기술하여 사용
...
SELECT DEPT_CODE, SUM(SALARY) FROM EMPLOYEE; -- 1행
...
-- EMPLOYEE 테이블에서 (FROM)
-- 직급코드가 같은 사람의 (직급을 급으로 묶기 : JOB_CODE)
...
SUM(SALARY) "급여 합"
/*1*/FROM EMPLOYEE
/*2*/GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') -- 별칭사용 X (SELECT 절 해석 x) 
...
GROUP BY DEPT_CODE; -- WHERE절에서 구한 부서코드가 D5, D6인 데이터증능 그룹으로 묶음
...
-- EMPLOYEE 테이블에서 (FROM)
-- 부서별로 같은 직급인 사원의 인원수를 조회 (GROUP BY DEPT_CODE, JOB_CODE 묶음)
-- 부서코드 오름차순, 직급코드 내림차순으로 정렬 (ORDER BY)
-- 부서코드, 직급코드, 인원수 (SELECT)
SELECT DEPT_CODE, JOB_CODE, COUNT(*)
FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE -- DEPT_CODE로 그룹을 나누고, 나눠진 그룹 내에서 JOB_CODE로 그룹 분류
