...
-- DEPARTMENT 테이브르이 DEPT_ID를 연결고리 지정
...
ON(DEPT_CODE = DEPT_ID); --24행(DEPT_CODE가 NULL인 하동운, 이오리 포함)
...
AND LOCATION_ID = LOCAL_CODE; --(EMPLOYEE + DEPARTMENT 조인) + LOCATION 조인
...
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
WHERE EMP_NO LIKE '7%'
AND SUBSTR(EMP_NO, INSTR(EMP_NO, '-')+1) LIKE '2%'
AND EMP_NAME LIKE '전%';
...
SELECT EMP_NAME, DEPT_TITLE, NATIONAL_NAME, LOCAL_NAME
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
