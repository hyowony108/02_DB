 ...
 * 
 * - IN / NOT IN : 여러개의 결과값 중에서 한개라도 한개라도 일치하는 값이 있다면 / 없다면
 * 
...
-- 3) 최종적으로 EMPLOYEE 테이블에서 위의 결과들과 동일한 DEPT_CODE를 동일한 사원 조회해라.
SELECT EMP_NAME, DEPT_CODE
FROM EMPLOYEE
...
--> 상관 서브쿼리를 사용하여 각 지원의 MANAGER_ID가 
...
-- 메인쿼리(부서별 입사일이 가장 빠른 사원의 사번, 이름, 부서코드, 부서명(NULL이면 '소속없음'))
...
FROM (SELECT EMP_NAME, SALARY
			FROM EMPLOYEE
			ORDER BY SALARY DESC) -- 해석순위 1위 FROM 절에서 전체 직원의 SALARY 내림차순
WHERE ROWNUM <= 5; -- 해석순위 2위 WHERE절에서 가상 컬럼의 1~5행까지만 조회
