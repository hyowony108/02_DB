...
-- DEPARTMENT 체이블을 복사한 DEPT_COPY2 생성 (테이블)
CREATE TABLE DEPT_COPY2
AS SELECT * FROM DEPARTMENT;

-- DEPT_COPY2 테이블에서 DEPT_ID, LOCATION_ID 컬러만 이용해
-- V_DCOPY2 VIEW 생성
...
-- 사용하지 않은 컬럼(DEPT_TITLE)에는 NULL이 들어감
--> NULL은 DB의 무결성을 약하게 만든느 주요 원인
-- 가능하면 의도되지 않은 NULL은 존재하지 않게 해야한다

/* 무결성 : 데이터 베이스에서 데이터를 정확하고 일관되게
 * 				 유지하기 위한 중요한 개념.
...
--> CURRAVAL의 정확한 의미는 가장 최근에 호출된 NEXTVAL의 값을 반환함을 뜻함
