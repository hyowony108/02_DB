...
-- 사번, 이름, 부서코드, 직급코드, 고용일
SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE, TO_CHAR(HIRE_DATE, 'RR/MM/DD')
FROM EMPLOYEE
WHERE (DEPT_CODE, JOB_CODE) = (
SELECT DEPT_CODE, JOB_CODE
FROM EMPLOYEE
...
TRUNC(ABS(MONTHS_BETWEEN(TO_DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD'), SYSDATE)) / 12) 나이, 
...
SELECT TRUNC(ABS(MONTHS_BETWEEN(TO_DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD'), SYSDATE)) / 12) 나이
FROM EMPLOYEE;
